...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, tokenizer: transformers.PreTrainedTokenizer, data_args):
        ...
        for data in dataset_list:
            annotation_paths = data["annotation_path"]
            ...
            if len(data_paths) == 1 and len(annotation_paths) > 1:
                data_paths = data_paths * len(annotation_paths)
            
            assert len(annotation_paths) == len(data_paths), "Annotation and data paths must have same length"

            for ann_path, dat_path in zip(annotation_paths, data_paths):
                ...
                if sampling_rate < 1.0:
                    ...
                    print(f"sampling {len(annotations)} examples from dataset {ann_path}")
                else:
                    rank0_print(f"dataset name: {ann_path}")
                
                for ann in annotations:
                    ann["data_path"] = dat_path
                    ann["tag"] = data["tag"]
    ...
    def _ensure_conversations(self, sample: Dict):
        """Ensure sample has a conversations field expected by the pipeline.

        Some datasets (e.g., raw R2R annotations) only provide `instructions`
        and `video`/`image` paths. For quick compatibility, synthesize a
        minimal conversation that places a visual token followed by the
        instruction text.
        """
        ...
        if "instructions" in sample:
            instr = " ".join(sample.get("instructions", [])).strip()
            ...
            if "actions" in sample:
                # Map each action integer to its string representation
                mapped_actions = []
                for a in sample["actions"]:
                    try:
                        a_int = int(a)
                        mapped_actions.append(action_map.get(a_int, "STOP"))
                    except ValueError:
                        # Fallback if action is not an integer
                        mapped_actions.append("STOP")
                answer = " ".join(mapped_actions)

            # Construct the complex prompt matching JanusVLN
            prompt = (
                "You are a visual language navigation model, and your should go to the locations to complete the given task. "
                "Compare the observation and instruction to infer your current progress, and then select the correct direction from the candidates to go to the target location and finish the task.\n "
                f"This is your historical and current observation: {DEFAULT_VIDEO_TOKEN}\n "
                f"Your task is to {instr}\n "
    ...
    def lengths(self):
        ...
        for sample in self.list_data_dict:
            sample = self._ensure_conversations(sample)
            cur_len = sum(
    ...
    def read_video_images(self, source):
        ...
        if os.path.isdir(video_file):
            ...
            rgb_dir = os.path.join(video_file, "rgb")
            frame_files = [os.path.join(rgb_dir,f) for f in os.listdir(rgb_dir) if os.path.isfile(os.path.join(rgb_dir, f))]
            frame_files.sort()
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        sources[0] = self._ensure_conversations(sources[0])
        ...
        if "image" in sources[0]:
            ...
            if isinstance(image_file, List):
                if len(image_file) > 1:
                    ...
                    #     pixel_values, thw = self.process_image_unified(file)
                    ...
                    pixel_values, grid_thw = self.process_image_unified(image_file)
                    image = [pixel_values]
            else:
                raise NotImplementedError
                # image_file = os.path.join(image_folder, image_file)
